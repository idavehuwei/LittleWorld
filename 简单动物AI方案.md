...
世界生成完成后，在自然地表确定性随机生成 5–10 只动物，动物类型为猪或鸡。动物平时在地表缓慢游荡并偶尔停下，玩家靠近到 3 米以内时向远离玩家的方向逃跑，持续一段时间后恢复游荡。
...
`NavigationAgent3D` 适合静态或低频变化的连续地形，能进行较长距离路径规划；但本项目允许玩家随时破坏和放置方块，静态 NavigationMesh 会迅速失效。对仅有 5–10 只、只需局部游荡和逃跑的动物，简单射线具有以下优势：
